...
下午 13:02、13:22、13:42、14:02、14:22、14:42、15:02、15:10
终局 20:04
```

`09:15` 是盘前启动与健康检查。Tushare 连续竞价分钟线从 `09:30` 开始，因此 09:15 不生成或覆盖预测；从 09:35 起使用当日实时分钟数据滚动计算。下午任务从 `13:02` 开始，确保午后首个分钟 bar 已形成，并避开 GitHub Actions 整点调度高峰。GitHub Actions 使用仓库 Secret `TUSHARE_TOKEN`，令牌不得写入代码、配置、日志或报告。
