...
<head>
  ...
  <style>
    .pair-table tbody{{display:block}}.pair-table tr{{display:grid;grid-template-columns:max-content minmax(0,1fr) max-content minmax(0,1fr);align-items:stretch}}.pair-table th,.pair-table td{{width:auto;min-width:0}}.pair-table th{{padding-right:0;white-space:nowrap}}.pair-table th::after{{content:"："}}.pair-table td{{padding-left:6px;overflow-wrap:anywhere}}@media(max-width:760px){{.pair-table tr{{grid-template-columns:max-content minmax(0,1fr)}}.pair-table tr:last-child>:nth-child(-n+2){{border-bottom:1px solid var(--line)}}}}
  </style>
</head>
<body>
