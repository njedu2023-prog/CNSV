...
def build_trading_markdown(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- 明确买卖建议: {d.get('trade_advice_cn', d['suggested_action'])}",
        f"- 空仓操作: {d.get('entry_advice', d['suggested_action'])}",
        f"- 持仓操作: {d.get('holding_advice', d['suggested_action'])}",
  ...
  <div class="hero">
    <div>
      ...
      <div class="decision">
        ...
        <div class="decision-text">次日{'看涨' if predicted_direction == 'UP' else '看跌' if predicted_direction == 'DOWN' else '方向不可用'} · 明确建议：{d.get('trade_advice_cn', d['suggested_action'])}</div>
        <div class="trade-advice" aria-label="明确买卖建议">
          <div><span>空仓操作</span><strong>{d.get('entry_advice', d['suggested_action'])}</strong></div>
          <div><span>已有仓位</span><strong>{d.get('holding_advice', d['suggested_action'])}</strong></div>
        </div>
        ...
        <div class="hero-grid">
          ...
          <div class="metric"><div class="label">明确买卖建议</div><div class="value long-text">{d.get('trade_advice_cn', d['suggested_action'])}</div></div>
        </div>
      </div>
    </div>
  </div>
