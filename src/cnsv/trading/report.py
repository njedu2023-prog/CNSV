...
<main>
  ...
  <section><h2>模型表现追踪</h2><div class="three-grid">
    <div class="metric"><div class="label">{historical_stats.get('name', '历史统计线')}</div><div class="value">{probability_pct(historical_stats.get('direction_accuracy'))}</div><p class="note">样本数：{fmt_count(historical_stats.get('sample_count'))}</p><p class="note">历史验证 / walk-forward / purged</p></div>
    <div class="metric"><div class="label">{live_stats.get('name', '实盘统计线')}</div><div class="value">{_live_accuracy_text(live_stats)}</div><p class="note">起始：{live_stats.get('start_date', '2026-06-21')} · 样本：{live_stats.get('sample_count', 0)}</p><p class="note">正确：{live_stats.get('correct_count', 0)} · 错误：{live_stats.get('wrong_count', 0)}</p></div>
    <div class="metric"><div class="label">历史验证补充</div><div class="value">B2 5D {probability_pct(b2_std.get('directional_accuracy', 0.0))}</div><p class="note">standard：{fmt_count(b2_std.get('sample_size'))} · purged：{probability_pct(b2_purged.get('directional_accuracy'))}</p><p class="note">P2 覆盖：{probability_pct(p2_std.get('terminal_p10_p90_coverage'))} · Brier：{fmt_number(p2_std.get('positive_terminal_brier'))}</p></div>
  </div></section>
