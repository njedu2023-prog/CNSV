...
<main>
  <header>
    <p class="eyebrow">CNSV V1.4 主线看板</p>
    <h1>中国船舶主线看板</h1>
    <p class="subtitle">展示 CNSVdata 准入、数据覆盖、特征质量、路径分布与 V1.4 观察级回测入口。页面不生成交易动作。</p>
    <nav><a href="#coverage">数据状态</a><a href="#priceVolumeCards">核心特征</a><a href="baseline.html">V1.2 基准模型</a><a href="validation.html">V1.2.2 验证</a><a href="path.html">V1.3 路径分布</a><a href="backtest.html">V1.4 观察级回测</a></nav>
  </header>
...
const labels = {
  ...
  trend: [
    ["trend_state", "趋势状态"], ["trend_ma5_ma20", "MA5-MA20"], ["trend_ma10_ma60", "MA10-MA60"], ["close_above_ma5", "收盘在 MA5 上方"], ["close_above_ma10", "收盘在 MA10 上方"], ["close_above_ma20", "收盘在 MA20 上方"], ["close_above_ma60", "收盘在 MA60 上方"], ["ma5_slope", "MA5 斜率"], ["ma10_slope", "MA10 斜率"], ["ma20_slope", "MA20 斜率"]
  ],
  volatility: [
    ["volatility_state", "波动率状态"], ["realized_vol_5d", "5D 实现波动率"], ["realized_vol_10d", "10D 实现波动率"], ["realized_vol_20d", "20D 实现波动率"], ["atr_14d", "ATR 14D"], ["intraday_range_ma5", "5D 日内振幅均值"], ["intraday_range_ma20", "20D 日内振幅均值"]
  ]
};
...
Promise.all([
  ...
  const meta = featureReport.meta || {};
  const dataMeta = dataReport.meta || {};
  const manifest = featureReport.data_manifest || dataReport.data_manifest || {};
  const summary = dataReport.loaded_data_summary || {};
  const gate = featureReport.cnsvdata_gate || dataReport.cnsvdata_gate || {};
  const quality = featureReport.feature_quality || {};
  const features = featureReport.features || dataReport.features || {};
  const price = features.price_volume || {};
  const moneyflow = features.moneyflow || {};
  const minute = features.minute_structure || {};
  ...
  document.getElementById("qualityDetails").innerHTML = detail(
    "质量检查明细",
    [chip("检查项", (quality.checks || []).length, "file_count")],
    (quality.checks || []).map(item => [checkName(item.name), `${statusText(item.status)} - ${checkDetail(item.detail)}`, ""])
  );
