...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    summary::after { content: "展开"; color: var(--blue); font-size: 15px; white-space: nowrap; font-weight: 600; }
...
fetch("data/latest_data_report.json")
  ...
  .then(data => {
    ...
    document.getElementById("features").innerHTML = [
      fold("量价摘要", [["收盘价", f.price_volume?.latest_close], ["涨跌幅", f.price_volume?.latest_pct_chg], ["MA20", f.price_volume?.ma20]], [["最新收盘价", f.price_volume?.latest_close], ["最新涨跌幅", f.price_volume?.latest_pct_chg], ["MA5", f.price_volume?.ma5], ["MA10", f.price_volume?.ma10], ["MA20", f.price_volume?.ma20], ["1D 收益", f.price_volume?.ret_1d], ["5D 收益", f.price_volume?.ret_5d], ["20D 收益", f.price_volume?.ret_20d], ["5D 成交量比", f.price_volume?.volume_ratio_5d], ["5D 成交额比", f.price_volume?.amount_ratio_5d]]),
