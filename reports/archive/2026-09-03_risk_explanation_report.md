...
- 总体风险等级: high
- 风险置信度: medium
- 主要风险来源: data_risk, feature_risk, p2_auxiliary_risk, evidence_conflict_risk
- 次要风险来源: baseline_model_risk, path_distribution_risk, path_validation_risk, observation_backtest_risk, decision_support_risk, system_boundary_risk
- 需要人工复核: True
- 风险原因: 风险等级由数据、特征、模型、路径、回测和人工辅助证据综合得到，仅用于人工复核。

## 风险来源拆解
- data_risk: high；数据门禁、数据新鲜度与 manifest 一致性需要人工确认。
- feature_risk: high；趋势、波动率、资金流状态共同决定特征风险；unknown 状态需要降级复核。
- baseline_model_risk: medium；基准模型分布与验证结果用于识别 B 模型之间的方向分歧。
- path_distribution_risk: low；路径下穿概率、最大回撤与路径波动率共同构成路径分布风险。
...
## 特征风险解释
- risk_level: low
- trend_state_risk: low；趋势状态为 strong_uptrend，需要结合路径风险复核。
- volatility_state_risk: medium；波动率状态为 low_vol。
...
- B2_state_sample_risk: medium；B2 最小状态样本数为 85。
- positive_prob_calibration_risk: medium；正向概率校准只能用于观察，不代表确定结果。
- quantile_coverage_risk: low；分位覆盖需要结合验证层。

## 路径风险解释
- risk_level: low
- downside_path_risk: low；20D 下行路径概率需要人工复核。
- touch_down_risk: low；20D touch_down_5pct_prob=0.3127。
- max_drawdown_risk: low；20D max_drawdown_p50=-0.0441。
...
- p2_fallback_risk: high；P2 max_fallback_rate=0.3151。
- p2_state_sample_risk: medium；P2 min_state_sample_size=85。
...
- risk_evidence: 20D P1 相对 P0 表现偏弱。, P2 fallback_rate 偏高，P2 只能作为辅助观察。, data_quality 状态为 WARN，需要降级复核。, feature_quality 状态为 WARN，需要降级复核。, path_quality 状态为 WARN，需要降级复核。
- source_reports: latest_human_decision_support_report.json
- human_review_required: True

## 风险情景卡片
- [downside_touch_risk_card] 下穿路径风险: low；下穿概率升高时，风险解释层要求人工复核路径尾部。
- [max_drawdown_risk_card] 最大回撤风险: low；历史路径回撤仅表示观察风险，不构成执行参数。
